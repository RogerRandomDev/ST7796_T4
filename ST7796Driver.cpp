...
ST7796Driver::ST7796Driver(uint8_t cs, uint8_t rs, uint8_t sid, uint8_t sclk, uint8_t rst)
{
	_cs   = cs;
	_rs   = rs;
	_sid  = sid;
...
ST7796Driver::ST7796Driver(uint8_t cs, uint8_t rs, uint8_t rst) 
{
	_cs   = cs;
	_rs   = rs;
	_rst  = rst;
...
FLASHMEM void ST7796Driver::updateScreenWithDiffs(const uint16_t* _new_fb){
  _diff1->computeDiff(_pfbtft,_new_fb, 0, _diff_gap, true, _compare_mask);
  _flush_cache(_pfbtft, 480*320 * 2);
...
FLASHMEM void ST7796Driver::drawPixel(int16_t x, int16_t y, uint16_t color)
{
	x += _originx;
	y += _originy;
...
FLASHMEM void ST7796Driver::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color)
{
	x+=_originx;
	y+=_originy;
...
FLASHMEM void ST7796Driver::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color)
{
	x+=_originx;
	y+=_originy;
...
FLASHMEM void ST7796Driver::fillScreen(uint16_t color)
{
	fillRect(0, 0,  _width, _height, color);
}

// fill a rectangle
FLASHMEM void ST7796Driver::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
	x+=_originx;
	y+=_originy;
...
FLASHMEM void ST7796Driver::setRotation(uint8_t m)
{
	...
	case 0:
     	if ((tabcolor == INITR_BLACKTAB) || (tabcolor == INITR_MINI160x80)) {
			writedata_last(MADCTL_MX | 0x00 | MADCTL_BGR);
		} else {
			writedata_last(MADCTL_MX | 0x00 | MADCTL_BGR);
		}
		_width  = _screenWidth;
...
///
// Draw a rectangle
FLASHMEM void ST7796Driver::drawRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
...
FLASHMEM void ST7796Driver::drawRoundRect(int16_t x, int16_t y, int16_t w,
...
FLASHMEM void ST7796Driver::fillRoundRect(int16_t x, int16_t y, int16_t w,
				 int16_t h, int16_t r, uint16_t color) {
  // smarter version
...
// Draw a triangle
FLASHMEM void ST7796Driver::drawTriangle(int16_t x0, int16_t y0,
...
// Fill a triangle
FLASHMEM void ST7796Driver::fillTriangle ( int16_t x0, int16_t y0,
				  int16_t x1, int16_t y1,
				  int16_t x2, int16_t y2, uint16_t color) {
...
FLASHMEM void ST7796Driver::drawCircle(int16_t x0, int16_t y0, int16_t r,
    uint16_t color) {
  int16_t f = 1 - r;
  int16_t ddF_x = 1;
  int16_t ddF_y = -2 * r;
...
FLASHMEM void ST7796Driver::drawCircleHelper( int16_t x0, int16_t y0,
...
FLASHMEM void ST7796Driver::fillCircle(int16_t x0, int16_t y0, int16_t r,
			      uint16_t color) {
...
FLASHMEM void ST7796Driver::fillCircleHelper(int16_t x0, int16_t y0, int16_t r,
...
FLASHMEM void ST7796Driver::drawLine(int16_t x0, int16_t y0,
...
FLASHMEM void ST7796Driver::drawBitmap(int16_t x, int16_t y,
...
FLASHMEM void ST7796Driver::setCursor(int16_t x, int16_t y, bool autoCenter) {
	_center_x_text = autoCenter;	// remember the state. 
	_center_y_text = autoCenter;	// remember the state. 
	if (x == ST7796Driver::CENTER) {
		_center_x_text = true;
		x = _width/2;
	}
	if (y == ST7796Driver::CENTER) {
		_center_y_text = true;
		y = _height/2;
	}
	if (x < 0) x = 0;
...
FLASHMEM void ST7796Driver::getCursor(int16_t *x, int16_t *y) {
  *x = cursor_x;
  *y = cursor_y;
}

FLASHMEM void ST7796Driver::setTextSize(uint8_t s_x, uint8_t s_y) {
    textsize_x = (s_x > 0) ? s_x : 1;
    textsize_y = (s_y > 0) ? s_y : 1;
}

FLASHMEM uint8_t ST7796Driver::getTextSize() {
	return textsize_x;	// bug bug 2 values now
}

FLASHMEM uint8_t ST7796Driver::getTextSizeX() {
	return textsize_x;
}
FLASHMEM uint8_t ST7796Driver::getTextSizeY() {
	return textsize_y;
}

FLASHMEM void ST7796Driver::setTextColor(uint16_t c) {
  // For 'transparent' background, we'll set the bg
  // to the same as fg instead of using a flag
  textcolor = textbgcolor = c;
}

FLASHMEM void ST7796Driver::setTextColor(uint16_t c, uint16_t b) {
  textcolor   = c;
  textbgcolor = b;
...
FLASHMEM void ST7796Driver::setTextWrap(boolean w) {
  wrap = w;
}

FLASHMEM boolean ST7796Driver::getTextWrap()
{
	return wrap;
}

FLASHMEM uint8_t ST7796Driver::getRotation(void) {
  return _rot;
}
...
***************************************************************************************/
FLASHMEM int16_t ST7796Driver::drawNumber(long long_num, int poX, int poY)
{
  char str[14];
...
int16_t ST7796Driver::drawFloat(float floatNumber, int dp, int poX, int poY)
{
...
int16_t ST7796Driver::drawString(const String& string, int poX, int poY)
{
...
int16_t ST7796Driver::drawString1(char string[], int16_t len, int poX, int poY)
{
  int16_t sumX = 0;
...
size_t ST7796Driver::write(uint8_t c) {
	return write(&c, 1);
}

size_t ST7796Driver::write(const uint8_t *buffer, size_t size)
{
...
// Draw a character
FLASHMEM void ST7796Driver::drawChar(int16_t x, int16_t y, unsigned char c,
...
FLASHMEM void ST7796Driver::setFont(const ST7796_T4_font_t &f) {
	font = &f;
	_gfx_last_char_x_write = 0;	// Don't use cached data here
	if (gfxFont) {
        cursor_y -= 6;
...
FLASHMEM void ST7796Driver::setFont(const GFXfont *f) {
...
FLASHMEM void ST7796Driver::drawFontChar(unsigned int c)
{
	uint32_t bitoffset;
	const uint8_t *data;
...
int16_t ST7796Driver::strPixelLen(const char * str)
{
...
FLASHMEM void ST7796Driver::charBounds(char c, int16_t *x, int16_t *y,
...
FLASHMEM void ST7796Driver::getTextBounds(const uint8_t *buffer, uint16_t len, int16_t x, int16_t y,
...
FLASHMEM void ST7796Driver::getTextBounds(const char *str, int16_t x, int16_t y,
        int16_t *x1, int16_t *y1, uint16_t *w, uint16_t *h) {
...
FLASHMEM void ST7796Driver::getTextBounds(const String &str, int16_t x, int16_t y,
        int16_t *x1, int16_t *y1, uint16_t *w, uint16_t *h) {
    if (str.length() != 0) {
...
FLASHMEM void ST7796Driver::drawFontPixel( uint8_t alpha, uint32_t x, uint32_t y ){
...
FLASHMEM void ST7796Driver::drawFontBits(bool opaque, uint32_t bits, uint32_t numbits, int32_t x, int32_t y, uint32_t repeat)
{
	if (bits == 0) {
		if (opaque) {
...
FLASHMEM void ST7796Driver::drawGFXFontChar(unsigned int c) {
...
FLASHMEM bool ST7796Driver::gfxFontLastCharPosFG(int16_t x, int16_t y) {
...
//=======================================================================
FLASHMEM void ST7796Driver::setFrameBuffer(uint16_t *frame_buffer) 
{
...
FLASHMEM uint8_t ST7796Driver::useFrameBuffer(boolean b)		// use the frame buffer?  First call will allocate
{
	if (b) {
...
FLASHMEM void ST7796Driver::freeFrameBuffer(void)						// explicit call to release the buffer
{
