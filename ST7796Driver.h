...
#define ST7796_T4_font_t ST7796_T4_font_t
...
class ST7796Driver : public Print
{
  ...
  ***********************************************************************************************************/
  ...
  volatile int _diff_gap=4;                     // gap when creating diffs.refresh).
  volatile uint16_t _compare_mask=0;             // the compare mask used to compare pixels when doing a diff

  ST7796_T4::DiffBuffBase* volatile  _diff1;             // first diff buffer
  ST7796_T4::DiffBuffBase* volatile  _diff2;             // second diff buffer (if non null, then _diff1 is also non zero). 
  ST7796_T4::DiffBuffDummy* volatile _dummydiff1;        // fake diff buffer used for complete refresh and when buffering is disabled. 
  ST7796_T4::DiffBuffDummy* volatile _dummydiff2;        // fake diff buffer used for complete refresh and when buffering is disabled. 

  ST7796_T4::DiffBuffDummy _dd1, _dd2;                    // the dummy diff themselves. 
  ...
  //changed
    ...
    void push_pixels(uint16_t* fb,int xx,int yy,int len);
...
//    digitalWriteFast(2, LOW);
  ...
  static  ST7796Driver     *_dmaActiveDisplay[3];  // Use pointer to this as a way to get back to object...
...
public:
	...
	void initButton(ST7796Driver *gfx, int16_t x, int16_t y,
		uint8_t w, uint8_t h,
